...
std::map<std::string,SalFile *> salPtrMap;
		...
		fprintf(stderr, "[%s] at (%f, %f)\n", euc_str, x, y);
	      */
	      ...
	      if((font2 = sdl_ttf2->getFontTextureSJIS((char *)xmlNodeGetContent(NM),
						       color2_)) == NULL)
		{
		  fprintf(stderr, "!sdl_ttf2->getFontTextureSJIS()\n");
		  exit(-1);
		}
...
void splash(char * str)
{
  ...
  if((font = sdl_ttf->getFontTexture(str, color_)) == NULL)
    {
      fprintf(stderr, "!sdl_ttf->getFontTexture(str, color_)\n");
      exit(-1);
    }
...
void display(void)
{
  ...
  if(viewer.getViewerMode() != PARALLEL)
    {
      MeshIV meshIV = me1.get_meshes(viewer.getLeftLongitude(), viewer.getRightLongitude(),
				     viewer.getTopLatitude(), viewer.getBottomLatitude());
      for(MeshIVI mi = meshIV.begin();mi != meshIV.end();mi ++)
	{
	  if(displayList.callList((*mi)->second.meshID) != 0)
	    {
#define D_ 1
	      double ** dvv = (*mi)->second.height;
	      double dvx = (*mi)->second.base_longitude;
	      double dvy = (*mi)->second.base_latitude;
	      
	      if(me1.is_complete_mesh(&(*mi)->second) == 0)
		displayList.newList((*mi)->second.meshID);
		...
		for(int ay = 0;ay < size_latitude;ay += D_)
	      ...
	      if(me1.is_complete_mesh(&(*mi)->second) == 0)
		displayList.endList((*mi)->second.meshID);
	    }
	}
    }
  
  if(viewer.getViewerMode() != MESHONLY)
    {
      for(std::vector<SLM>::iterator slmi = slmV.begin();slmi != slmV.end();slmi ++)
	{
	  if(displayList.callList((char *)slmi->city_number.c_str()) != 0)
	    {
	      std::map<std::string,SalFile *>::iterator pi = salPtrMap.find(slmi->city_number);
	      if(pi != salPtrMap.end())
		{
		  displayList.newList((char *)slmi->city_number.c_str());
		  disSAL(pi->second);
		  // too slow
		  //glDisable(GL_BLEND);
		  //disCM(pi->second);
		  ...
		  displayList.endList((char *)slmi->city_number.c_str());
		  delete pi->second;
		  salPtrMap.erase(pi);
		}
	    }
  ...
  if((font = sdl_ttf->getFontTexture(fff, color_)) == NULL)
    {
      fprintf(stderr, "!sdl_ttf->getFontTexture(fff, color_)\n");
      exit(-1);
    }
  glBindTexture(GL_TEXTURE_2D, font->textureID);
  glBegin(GL_POLYGON);
  ...
  char PERS[] = "透視", PARA[] = "平行", MESHS[] = "標高";
  if(viewer.getViewerMode() == PERSPECTIVE)
    sprintf(fff,"<%s>", PERS);
  else if(viewer.getViewerMode() == PARALLEL)
    sprintf(fff,"<%s>", PARA);
  else if(viewer.getViewerMode() == MESHONLY)
    sprintf(fff,"<%s>", MESHS);
  ...
  if((font2 = sdl_ttf2->getFontTextureEUCJP(fff, color2_)) == NULL)
    {
      fprintf(stderr, "!sdl_ttf2->getFontTextureEUCJP(fff, color2_)\n");
      exit(-1);
    }
  glBindTexture(GL_TEXTURE_2D, font2->textureID);
  glBegin(GL_POLYGON);
  ...
  sprintf(fff, "Total SLM: %d", slmV.size());
  SDL_Color color_4 = {200, 145, 255, 0};
  glEnable(GL_TEXTURE_2D);
  glColor3d(1, 1, 1);
  if((font = sdl_ttf->getFontTexture(fff, color_4)) == NULL)
    {
      fprintf(stderr, "!sdl_ttf->getFontTexture(%s, color_4)\n", fff);
      exit(-1);
    }
  glBindTexture(GL_TEXTURE_2D, font->textureID);
  glBegin(GL_POLYGON);
  ...
  glVertex3f(0, 100, 1);
  glTexCoord2f(1, 0);
  glVertex3f(0+font->width/2, 100, 1);
  glTexCoord2f(1, 1);
  glVertex3f(0+font->width/2, 100+font->height/2, 1);
  glTexCoord2f(0, 1);
  glVertex3f(0, 100+font->height/2, 1);
  glEnd();
  glDisable(GL_TEXTURE_2D);
...
int thread_keyboard(void *data)
{
  while(global_thread_data1 == 0)
      ...
      for(std::vector<SLM>::iterator i = slmV.begin();i != slmV.end();i ++)
	{
	  ...
	  sprintf(url,
		  "http://133.11.205.121/GSI/sdf.gsi.go.jp/data25k/%s/%s.lzh#%s/%s", 
		  //"http://sdf.gsi.go.jp/data25k/%s/%s.lzh#%s/%s",
	  ...
	  if(slmset->isSLMLoaded(c_i) != 1)
	    {
	      ...
	      salPtrMap.insert(std::pair<std::string,SalFile *>(i->city_number,new SalFile(url)));
	    }
	}
    }
  return 0;
...
{
  fprintf(stderr, "\n");
  fprintf(stderr, "GISfx "VERSION"\n");
  fprintf(stderr, "SDL(%d)\n", SDL_COMPILEDVERSION);
  fprintf(stderr, "Build Date: "__DATE__"\n");
  ...
  if(viewer.sdl_init_video(0, 0) != 0)
    {
      SDL_Quit();
      fprintf(stderr, "viewer.sdl_init_video(0, 0) == 0\n");
      exit(2);
    }
  SDL_WM_SetCaption("GISfx", "gisfx");

  texture = new TextureImage();
  sdl_ttf = new TextureSDL_ttf("/usr/X11R6/lib/X11/fonts/TTF_j/VeraBd.ttf", 50);
  //sdl_ttf2 = new TextureSDL_ttf("/usr/X11R6/lib/X11/fonts/truetype_j/kochi-gothic.ttf", 30);
  sdl_ttf2 = new TextureSDL_ttf("/usr/X11R6/lib/X11/fonts/TTF_j/VeraBd.ttf", 30);
